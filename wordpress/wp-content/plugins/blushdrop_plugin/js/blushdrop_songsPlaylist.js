    ...
    if (supportsAudio) {
        audioPlayer = {
            ...
            loadTrack: function (list) {
                ...
                inf.cover.attr({'background-image': data.cover});
            },
            play: function (data) {
                this.playing = true;
            ...
            next: function () {
                ...
                if(data.index == 9){
                    //call ajax and build again
                }
                else{
                    var brother = $loaded.next();//find the next brother
                    this.loadTrack(brother)
                    (this.playing)? this.play(data) : this.pause(data); //or null if falsy?
                }
            },
            prev: function () {
                var $loaded = $(this.loaded);
                ...
                if(data.index == 0){
                    ...
                    var brother = $loaded.prev();//find the next brother
                    this.loadTrack(brother)
                    (this.playing)? this.play(data) : this.pause(data); //or null if falsy?
                }
            },
        ...
        audioPlayer.playlist.on('click', ':checkbox', function (e) {
            audioPlayer.playlist.find(':checkbox').not(this).prop('checked', false);
        });
    }
});