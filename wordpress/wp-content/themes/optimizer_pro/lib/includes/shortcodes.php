...
if(class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'shortcodes' ) ){
		...
		if(is_customize_preview()){ $nonedit = 'mceNonEditable blockshortcode';}else{$nonedit = '';}
			...
			$return_html = '<div style="clear:both;"></div><div class="ast_vid '.$class.' '.$nonedit.'"><div class="responsive-container"><'.$node.' class="vid_iframe" style=" width: '.$width.'; height: '.$height.';" src="'.$content.''.$autoplay.'" allowfullscreen></'.$node.'></div></div>';
...
function layersc_col2_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		if(is_customize_preview()){ 
			...
			$short = 'contenteditable="true" data-shortcode="[col2 width=&quot;'.$width.'&quot;]'.str_replace('"', '&quot;', $content).'[/col2]"';
...
function layersc_col3_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		if(is_customize_preview()){ 
			$nonedit = 'inline_shortcode';
			$short = 'contenteditable="true" data-shortcode="[col3 width=&quot;'.$width.'&quot;]'.str_replace('"', '&quot;', $content).'[/col3]"';
...
function layersc_col4_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		if(is_customize_preview()){ 
			$nonedit = 'inline_shortcode';
			$short = 'contenteditable="true" data-shortcode="[col4 width=&quot;'.$width.'&quot;]'.str_replace('"', '&quot;', $content).'[/col4]"';
...
function layersc_button_func($atts, $content = null) {
	...
	if ($open_new_window == 'false'){
		...
		if(is_customize_preview()){ 
			$nonedit = 'inline_shortcode';
			$short = 'data-shortcode="[button text=&quot;'.$text.'&quot; url=&quot;'.$url.'&quot; background_color=&quot;'.$background_color.'&quot; text_color=&quot;'.$text_color.'&quot; style=&quot;'.$style.'&quot; size=&quot;'.$size.'&quot; icon=&quot;'.$icons.'&quot; open_new_window=&quot;'.$shortwindow.'&quot; rounded=&quot;'.$shortround.'&quot;]"';
			$inline = 'data-btn-text="'.$text.'"  data-btn-url="'.$url.'" data-btn-bg="'.$background_color.'" data-btn-color="'.$text_color.'" data-btn-style="'.$style.'" data-btn-size="'.$size.'" data-btn-icon="'.$icons.'"  data-btn-window="'.$shortwindow.'"  data-btn-rounded="'.$shortround.'"  data-btn-class="'.$class.'"   ';
			$href ='';
		}else{
	...
	$return_html = '<a class="lts_button lts_button_'.$size.' '.$roundeds.' '.$style.' '.$class.' '.$nonedit.'" '.$open_new_windows.' style="background:'.$background_color.';color:'.$text_color.';border-color:'.$background_color.';" '.$href.' '.$short.' '.$inline.'>'.$icons .$text.'</a>';
...
function layersc_carousel( $atts, $content = null ) {
	...
	if(is_customize_preview()){ $nonedit = 'mceNonEditable blockshortcode';}else{$nonedit = '';}
		...
		return '
		<div class="looper slide lts_looper lts_'.$style.' '.$class.' '.$nonedit.'" data-looper="go" data-buttoncolor="'.$button_color.'" style="background:'.$background.';color:'.$text_color.'">
...
function layersc_lightbox_func($atts, $content) {
	...
	$return_html = '<span style="'.$button_text_color. $button_text_bg .$button_font_size.'" class="lts_lightbox_bttn lt_animate '.$rounded.' '.$button_style.' '.'lts_button_'.$button_size.' '.$class.' '.$nonedit.'">'.$button_text.'</span><span style="display:none" class="lts_lightbox_content">'.do_shortcode($content).'</span>';
