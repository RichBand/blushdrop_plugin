	...
	function widget( $args, $instance ) {
		...
		if(is_customize_preview()){
			...
			echo '<script>jQuery(window).bind("load", function(){
				...
				echo '<style>#'.$id.'{ ' . $content_bg . '' . $content_color . ''.$content_bgimg.'}#'.$id.' .widget_wrap, #'.$id.' .widget_wrap .widgettitle{' . $content_color . '}#'.$id.' .widget_wrap .ast_countdown li{color:rgba('.optimizer_hex2rgb($instance['content_color']).', 0.8)!important;}</style>';
	...
	<?php
		...
		if ( empty( $settings ) ) {
			...
			foreach ( $settings as $instance_id => $instance ) {
				...
				$widget_style = '#'.$id.'{ ' . $content_bg . '' . $content_color . '' . $content_bgimg . '}#'.$id.' .widget_wrap, #'.$id.' .widget_wrap .widgettitle{' . $content_color . '}#'.$id.' .widget_wrap .ast_countdown li{color:rgba('.optimizer_hex2rgb($content_rgba).', 0.8)!important;}';
				wp_add_inline_style( 'optimizer-style', $widget_style );
			}
		}
	} //END FOREACH
...
class optimizer_pinterest_Widget extends WP_Widget {
    ...
    function get_pins($username, $num) {
        ...
        if (is_null($rss_items)) {
            ...
            $search = '236x';
			if($size == 'large'){$replace = '736x';}else{$replace = '236x';}
