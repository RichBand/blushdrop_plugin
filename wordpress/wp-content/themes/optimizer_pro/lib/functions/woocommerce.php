...
function optimizer_wcmenucart_update( $fragments ) {
	ob_start();
	?>
   <a class="wcmenucart-contents" href="<?php echo WC()->cart->get_cart_url(); ?>" title="<?php _e( 'View your shopping cart' ); ?>"><?php echo sprintf (_n( '%d item', '%d items', WC()->cart->get_cart_contents_count() ), WC()->cart->get_cart_contents_count() ); ?> - <?php echo WC()->cart->get_cart_total(); ?></a>
	<?php
	
	$fragments['a.wcmenucart-contents'] = ob_get_clean();
	if ( WC()->cart->get_cart_contents_count() == 0 ) {
			echo '<li id="optimizer_minicart_wrap"><div id="optimizer_minicart">'.__('No Item found on your cart.', '').'</div></li>';
		}else{
