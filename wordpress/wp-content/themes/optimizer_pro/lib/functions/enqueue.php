...
function optimizer_css_js() {
	...
	wp_enqueue_script('optimizer_js',get_template_directory_uri().'/assets/js/optimizer.js', array('jquery'),'1.0', true);
	wp_enqueue_script('optimizer_otherjs',get_template_directory_uri().'/assets/js/other.js', array('jquery'),'1.0', true);
	wp_enqueue_script('optimizer_core',get_template_directory_uri().'/assets/js/core.js', array('jquery'),'1.0', true);
		$optim = array(
	...
	global $optimizer; if ( ! empty ( $optimizer['post_lightbox_id'] ) ) {wp_enqueue_script('optimizer_lightbox',get_template_directory_uri().'/assets/js/magnific-popup.js', array('jquery'),'1.0', true);}
	global $optimizer; if($optimizer['slider_type_id'] == "accordion"){wp_enqueue_script('optimizer_accordion',get_template_directory_uri().'/assets/js/accordion.js', array('jquery'),'1.0', true);}
	//Load MASONRY
	global $optimizerdb;
	if ($optimizer['cat_layout_id'] == "3" ) {
		if (!is_home() ){
			wp_enqueue_script('optimizer_masonry',get_template_directory_uri().'/assets/js/masonry.js', array('jquery'),'1.0', true);
		}
	}
	...
	if ( is_page() || is_single() ) {
		...
		if (!empty( $optimizer['post_gallery_id'] ) && optimizer_has_shortcode( $content, 'gallery' ) ) {
			wp_enqueue_script('optimizer_gallery',get_template_directory_uri().'/assets/js/gallery.js', array('jquery'),'1.0', true);
		}
	}
	if(is_customize_preview()){ wp_enqueue_script('optimizer_map',get_template_directory_uri().'/assets/js/map-styles.js', array('jquery'),'1.0', true); }
