	...
	function widget( $args, $instance ) {
		...
		if(!isset($instance['custom_testi'])){ echo '<p class="widget_warning" style="color: #fff;">'.__('Please Click the "+ Add New" button to create new testimonials','optimizer').'</p>';}
					...
					if(!empty($twitter_testi_on)){
						...
						if( $testi_layout == 'col1') { $looper = 'data-looper="go" class="looper slide" data-interval="8000"'; }else{ $looper = ''; }
		...
		if(is_customize_preview()){
			...
			echo "<script>jQuery(window).ready(function() { jQuery('#".$id." .testi_col1 .looper-inner li:eq(0)').addClass('active').css({'display':'none'});  });</script>";
			...
			echo '<style>#'.$id.'{ ' . $content_bg . $content_bgimg. '}#'.$id.' .home_title, #'.$id.' .home_subtitle, #'.$id.' span.div_middle{ color:' . $title_color . '} #'.$id.' .testi_content, #'.$id.' .testi_author a, #'.$id.' .testi_occu{color:'.$title_color.'opacity:0.7;}#'.$id.' span.div_left, #'.$id.' span.div_right{background-color:' . $title_color . '}</style>';			
...
<?php
		...
		foreach ( $settings as $instance_id => $instance ) {
			...
			$widget_style = '#'.$id.'{ ' . $content_bg . $content_bgimg. '}#'.$id.' .home_title, #'.$id.' .home_subtitle, #'.$id.' span.div_middle{ color:' . $title_color . '} #'.$id.' .testi_content, #'.$id.' .testi_author a, #'.$id.' .testi_occu{color:'.$title_color.'opacity:0.7;}#'.$id.' span.div_left, #'.$id.' span.div_right{background-color:' . $title_color . '}';
