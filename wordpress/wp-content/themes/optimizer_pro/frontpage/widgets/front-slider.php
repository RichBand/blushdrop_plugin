...
class optimizer_front_Slider extends WP_Widget {
	...
	function widget( $args, $instance ) {
				  ...
				  echo '<div class="the_slider_widget" data-pausetime="'.$slider_pausetime.'">';
						  ...
						  foreach ( $attachments as $attachment ) {
							  ...
							  if(!empty($slider_caption) && $slider_type !== 'nivo'){ $slidedata = 'data-slide-desc="'.do_shortcode($attachment->post_excerpt).'" data-slide-button="'.do_shortcode(get_post_meta($attachment->ID, '_wp_attachment_image_alt', true)).'" data-slide-link="'.do_shortcode(esc_url($attachment->post_content)).'"';}else{$slidedata ='';}
								...
								echo '<img src="'.$imgsrc[0].'" width="'.$imgsrc[1].'" height="'.$imgsrc[2].'" alt="'.$attachment->post_title.'" title="'.$slide_title.'" '.$slidedata.' '.$slide_thumb.' />';
						  }
						  //FOR EACH ENDS
					echo '</div>';
				  }
		...
		if(is_customize_preview()){
			...
			echo '<style>#'.$id.' .widget_slider_content, #'.$id.' .nivo-html-caption{' . $content_color . '}#'.$id.' .widget_slider_content, #'.$id.' #opt_carousel .slidee li, #'.$id.' .widget_slider_content, #'.$id.' #opt_carousel .slidee li img, #'.$id.' #accordion, #'.$id.' #slide_acord, #'.$id.' .kwicks li{' . $slider_height . '}</style>';
		}
...
<?php
	...
	function optimizer_slider_widget_js(){
		...
		foreach ( $settings as $instance_id => $instance ) {
			...
			$widget_style = '#'.$id.' .widget_slider_content, #'.$id.' .nivo-html-caption{' . $content_color . '}#'.$id.' .widget_slider_content, #'.$id.' #opt_carousel .slidee li, #'.$id.' .widget_slider_content, #'.$id.' #opt_carousel .slidee li img, #'.$id.' #accordion, #'.$id.' #slide_acord, #'.$id.' .kwicks li{' . $slider_height . '}';
