...
function optimizer_in_widget_form($t,$return,$instance){
    $instance = wp_parse_args( (array) $instance, array( 'width' => 'none') );
        ...
        <div class="widget_advanced advanced_widget_toggle_off">
            ...
            <div class="widget_advanced_controls">
                ...
                <select id="<?php echo $t->get_field_id('width'); ?>" name="<?php echo $t->get_field_name('width'); ?>">
                    <option <?php selected($instance['width'], '1');?> value="1"><?php _e('Full','optimizer'); ?></option>
                    <option <?php selected($instance['width'], '2');?>value="2"><?php _e('Half','optimizer'); ?></option>
                    <option <?php selected($instance['width'], '3');?> value="3"><?php _e('One Third','optimizer'); ?></option>
                    <option <?php selected($instance['width'], '4');?> value="4"><?php _e('Three Quarter','optimizer'); ?></option>
                </select>
                </label>
            </div>
        </div>
...
function optimizer_dynamic_sidebar_params($params){
    ...
    if (isset($widget_opt[$widget_num]['width'])){
            if(isset($widget_opt[$widget_num]['width']))
                	...
                	$width = '';
            $params[0]['before_widget'] = preg_replace('/class="/', 'class="widget_col_'.$width.' ',  $params[0]['before_widget'], 1);
    }
    return $params;
}
