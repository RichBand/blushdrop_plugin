	...
	function widget( $args, $instance ) {
		...
		echo '<div class="optimizer_video_wrap video_'.$contentposition.' '.$border.'">';
			...
			if(!empty($customvdo )){
				...
				if( $autoplay == '1' && $contentposition == 'on_video'){ $hidecontrols ='hidecontrols'; $loop='true';  }else{ $hidecontrols ='';  $loop='false'; }
				...
				if(strpos($video_uri, 'youtu.be') !== false || strpos($video_uri, 'youtube.com') !== false){
					...
					if(!empty($vdothumb )){
						...
						if($contentposition == 'on_video' || $contentposition == 'top'){ 
							$thumbid = 'http://img.youtube.com/vi/'.$id.'/maxresdefault.jpg';
						}else{
							$thumbid = 'http://img.youtube.com/vi/'.$id.'/hqdefault.jpg';
						}
					}
